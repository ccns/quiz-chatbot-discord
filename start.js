...
var token = 'MzUzMTM2MDQ4MjgyMjcxNzQ0.DJZ0QQ.eqoEN30I22M7zHOmNwQERS1K7KY'
...
class MyClient {
    ...
    responseQuestion(message) {
        ...
        backendConnector.getQuestion(message.author.id)
            .then((question) => {
                backendConnector.userPrevQuestion[message.author.id] =
                    question.id
                message.reply(JSON.stringify(question))
            })
    }
}

var client = new MyClient(token)
client.backendConnector = backendConnector
